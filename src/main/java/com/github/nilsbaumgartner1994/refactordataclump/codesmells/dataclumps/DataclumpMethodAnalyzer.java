...
public class DataclumpMethodAnalyzer {
    ...
    public static void compareFunctionToAllCachedFunctions(JSFunction currentFunction, ProblemsHolder holder){
...
//            if(!otherFunction.equals(currentFunction)){
                DataclumpMethodAnalyzer.compareFunctionToOtherFunction(currentFunction, otherFunction, holder);
            }
        }
    }

    public static void compareFunctionToOtherFunction(JSFunction currentFunction, JSFunction otherFunction, ProblemsHolder holder){
        ...
        List<JSParameterListElement> commonParameters = PsiUtils.getCommonParameters(currentParameters, otherParameters);
        if(commonParameters.size() > 0){
            String description = "" + commonParameters.size() + " Parameters in " +
                    " file: " + currentFunction.getContainingFile().getVirtualFile().getUrl() +
                    " line : " + currentFunction.getTextOffset() +
                    ", method: " + currentFunction.getQualifiedName();
            JSParameterList parameterList = currentFunction.getParameterList();
            TextRange functionWithParametersRange = new TextRange(0, parameterList.getTextRangeInParent().getEndOffset());

            DataclumpAnalyzer.registerMethodProblem(holder, currentFunction, functionWithParametersRange, description);
        }
    }
